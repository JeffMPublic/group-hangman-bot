<?php
  ...
  require 'hangman.php'; // functions for the hangbot to play a game of hangman
  ...
  ************************************************** */
  ...
  /* **************************************************
    USER VARIABLES
  ************************************************** */
  $botId   = "c09a3974506424b8f18f7950da";
  ...
  ************************************************** */
  ...
  if (!iStr($p['name'],array('WordBot'))) { // This post wasn't made by the bot, so we'll continue (prevents infinite loop)
  ...
  if (strlen(trim($p['text'])) == 1) {
    ...
    if (iStr($text,array("wordbot","word bot","hangbot5000"))) {
      ...
      ************************************************** */
      ...
      elseif (iStr($text,array("hangman","word game","hang man","wordgame"))){
		sleep(1);// group me some times takes a while to display messages
		$strResponse = "Okay wait while I build us a game.\n";
		$groupMe->botPost($groupId,$botId,$p['name']." ".$strResponse); 
		$strResponse = "";
		if(!buildnewgame($p['user_id'])){
			 $strResponse .= "You already have an open game.\n";	
		}
		$strResponse .= displayGame($p['user_id']);
